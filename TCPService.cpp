...
void *TCPService::pthread_TCPServiceThread(void *) {
...
//    int cliSock[MAXCLIENT];          // Client Socket Set
  ...
  sleep(5);//wait for other thread start.
...
//  portNo = smem.GetLocalIP1(5);// atoi( argv[2] );
  ...
  // Create port socket
  tcpServSock = _tcpservice.CreateTCPServiceSocket(5000);
//    udpServSock = _tcpservice.CreateUDPServerSocket( portNo );
...
//                        cliSock[i]
            smem.setCliSock(i, _tcpservice.AcceptTCPConnection(tcpServSock));
...
//                        cliSock[i];
        ...
        if (_tcpservice.HandleTCPClient(events[i].data.fd)
            == 0) {
...
void TCPService::tcp_thread_generate() {
  pthread_t tid;
  ...
  pthread_create(&(tid), &attr, &(TCPService::pthread_TCPServiceThread), NULL);
  //pthread_record=tid;
  pthread_attr_destroy(&attr);

}

int TCPService::CreateTCPServiceSocket(unsigned short port) {
...
int TCPService::AcceptTCPConnection(int servSock) {
...
TCPService::HandleTCPClient(int clntSocket) {
  ...
  if (recvMsgSize > 0)      // zero indicates end of transmission
  {
    ...
    /* printf("receive message  =");
     ...
     printf("\n");*/                                //OT20110526
    smem.vWriteMsgToDOM("receive ShirinkAPP Request\n");
...
bool TCPService::parsTCP_JsonV3Content(char buff[4096],
                                       int connfd) {
  ...
  try {
    ...
    if (reader.parse(buff, JsonObjForAp)) {
      ...
      if (JsonObjForAp.isObject() && JsonObjForAp.isMember("weekdaysegment")) {
        ...
        printf("%s\n",
               JsonObjForAp["ActuateDTO"].toStyledString().c_str());
        smem.vWriteMsgToDOM("Received ActuateDTO\n");

        shirink_app::ActuateSetStruct ActuateObj;
        Json::Value JsonObj = JsonObjForAp["ActuateDTO"];
        ActuateObj.usActplayNo = JsonObj["ActNum"].asInt();
        for (int PedTriggerIndex = 0; PedTriggerIndex < 8; PedTriggerIndex++) {
          ActuateObj.valueSwitch[PedTriggerIndex] =
              JsonObj["PhaseTriggerSwitch"][PedTriggerIndex].asInt();
          ActuateObj.valueExtendSec[PedTriggerIndex] =
              JsonObj["PhaseExtendSec"][PedTriggerIndex].asInt();
        }
        ActuateObj.usiActType = JsonObj["ActType"].asInt();
        ActuateObj.usiActuatePlan = JsonObj["ActPlan"].asInt();
...
shirink_app TCPService::getTcData() {
  try {
